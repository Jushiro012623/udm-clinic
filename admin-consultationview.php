...
if(isset($_SESSION['system_adm'])){
	echo "<script> location.replace('sys-consultationview.php'); </script>";
	exit();
}
if(!isset($_SESSION['admin'])){
	...
	$sql = "SELECT * FROM stud_consultation_vw WHERE student_no = '$student_no' AND illness_code='$illness_code' ANd date_prepared='$date_prepared'";
...
if(isset($_POST['consult']))
{
	...
	$date_consulted = $row['date_consulted'];

	$sql = "UPDATE consultation SET attending_physician = '$attending_physician', date_consulted = CURRENT_TIMESTAMP, diagnosis = '$diagnosis'
            WHERE student_no = '$student_no' AND illness_code = '$illness_code' AND date_prepared = '$date_prepared' AND date_consulted = '0000-00-00 00:00:00'";
    if (mysqli_query($conn, $sql)){
		$sql = "INSERT INTO log (employee_no, role, time_log, action)
						VALUES('$employee_no', '$role', CURRENT_TIMESTAMP, 'Consulted a record')";
		if(mysqli_query($conn, $sql)){
			echo '<script>alert("Successfully consulted a record");window.location.href = "admin-consultation.php?consultrecord=1";</script>';
		}else{
			echo '<script>alert("Consult record failed");window.location.href = "admin-consultation.php?consult=fail";</script>';
		}
	}else{
		echo '<script>alert("Error Consulting a record");window.location.href = "admin-consultation.php?consultfail=1";</script>';
	}
...
/*START OF MODAL*/
.content .modal .modal-body .information-fillup .personal-info-fillup{ /*field set*/
...
#modal-fielset-div .modal-addrecord-info label{
	font-weight: lighter;
	font: 11.5px Arial, Helvetica, sans-serif;

}
#modal-fieldset .modal-addrecord-info label,
#modal-fieldset-inputed .modal-addrecord-info label{
	font-weight: lighter;
	margin-bottom:2px;
	font: 11.5px Arial, Helvetica, sans-serif;
...
#modal-fieldset-inputed .modal-addrecord-info{
	width: 32%;
}
#modal-fieldset-inputed .modal-addrecord-info span, 
...
#modal-fieldset .modal-addrecord-info input	{
	padding-left:.8em;
	color:#41a8b1;
}
select{
							...
							<fieldset class="consultation-info" id="modal-fieldset">
								...
								<div class="modal-consult-div">		
									<div class="modal-addrecord-info" id="attending-info">
										...
										<input style="color:#41a8b1;"id="inputed-illnesscode" name="user_attending" value="<?php echo $_SESSION['full_name'];?>" readonly></input>	
									</div>
								...
								<div class="modal-footer">
										<button  onclick="window.location.href='/clinicupdate/admin-consultation.php'" value = "back" name="back" class="buttonmodal" id="backbttn">Back</button>
