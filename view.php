...
if(isset($_POST['consult']))
{
	...
	$sql = "UPDATE medical_requirement SET status = 'consulted' WHERE student_no = '$student_no' 
		...
		if(mysqli_query($conn, $sql)){
			...
			if(mysqli_query($conn, $sql)){
			echo "<script>alert('Consult record success');window.location.href = 'admin-medical.php?consult=success';</script>";
			}else{
				echo "<script>alert('Consult record failed');window.location.href = 'admin-medical.php?consult=fail';</script>";
			}
		}else{
			echo "<script>alert('Update record failed');window.location.href = 'admin-medical.php?updaterecord=error';</script>";
		}
	}else{
		echo "<script>alert('Consult record failed');window.location.href = 'admin-medical.php?colnsultrecord=error';</script>";
	}
}
?>
